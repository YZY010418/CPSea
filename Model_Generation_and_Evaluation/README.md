...
Now, we can do inference. First, modify `configs/learn_angle.yaml`, change the structure_dir, dataset_dir and name of the generate dataset (create a new generate daraset besides exsited train and val dataset). Second, make an output diectory (since the script does not create the directory itself). Third, delete the torch_scatter import in `data/utils.py` because this is not in environment and is not used in the script. Finally, delete the repeated args.num_samples in `models_con/inference.py`. Then run:
...
```
python Evaluation/check_qtm_max_average.py Diff_Final_foldseek_novel/out --ignore_R --ignore_same_id
```
