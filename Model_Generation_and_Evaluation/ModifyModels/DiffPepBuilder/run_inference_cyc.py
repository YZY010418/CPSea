...
class PepGenDataset(PdbDataset):
    def __init__(
            ...
            *,
            ...
            sample_conf,
            diffuser,
            csv_file_path  # for assigning peptide length
    ):
        super().__init__(data_conf=data_conf, diffuser=diffuser, is_training=False)
        self.sample_conf = sample_conf
        self.csv_file_path = csv_file_path
        self.pdb_length_df = pd.read_csv(csv_file_path)  # Read the CSV file
    ...
    def __getitem__(self, idx):
        ...
        # Get length from self.pdb_length_df according to name
        length_row = self.pdb_length_df[self.pdb_length_df['pdb_name'] == pdb_name]
        if length_row.empty:
            raise ValueError(f"No length information found for PDB {pdb_name} in the length CSV.")
        peptide_len = length_row['length'].values[0]
        ...
        # Generate chain indices and sequence indices for the complex
...
class Sampler(Experiment):
    ...
    def __init__(
            self,
            conf: DictConfig,
            csv_path: str,  # Add csv_path parameter
    ):
        super().__init__(conf=conf)
        self.csv_path = csv_path  # Save csv_path as instance variable
    ...
    def run_sampling(self):
        ...
        if torch.cuda.is_available() and self._exp_conf.use_gpu:
            ...
            # Multi gpu mode
                ...
                if self._use_ddp:
                    ...
                    self._log.info(
                        f"Multi-GPU sampling on GPUs in DDP mode, node_id : {self.ddp_info['node_id']}, devices: {device_ids}")
                # DP mode
                else:
        ...
        # Use the CSV path from command line argument
        ...
        test_loader = du.create_data_loader(
            ...
            np_collate=False,
            length_batch=False,
            batch_size=self._exp_conf.eval_batch_size if not self._use_ddp else self._exp_conf.eval_batch_size // self.ddp_info[
                'world_size'],
            shuffle=False,
        ...
        for test_feats, pdb_names in test_loader:
            ...
            for i in range(batch_size):
                ...
                self._log.info(
                    f'Done sample {pdb_name} (peptide length: {peptide_len}, sample: {sample_id}), saved to {saved_path}')
...
def main(conf: DictConfig) -> None:
    ...
    parser = argparse.ArgumentParser(description='Run de novo peptide generation with custom CSV path')
    parser.add_argument('--csv_path', required=True, help='Path to the CSV file containing peptide length information')
    args = parser.parse_args()
    
    sampler = Sampler(conf=conf, csv_path=args.csv_path)
