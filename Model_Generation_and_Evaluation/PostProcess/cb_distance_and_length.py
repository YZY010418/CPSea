...
def calculate_cb_distance_and_chain_length(structure):
    target_chain = None
...
def main():
    parser = argparse.ArgumentParser(description='Calculate CB distances and L chain lengths for PDB files in a directory.')
    ...
    for file_path in tqdm(pdb_files, desc="Processing PDB files"):
        try:
            ...
            distance, chain_length = calculate_cb_distance_and_chain_length(structure)
            if distance is not None and chain_length is not None:
                results.append((file_path, distance, chain_length))
        except Exception as e:
            print(f"Error processing {os.path.basename(file_path)}: {e}")

    with open(args.output_csv, 'w') as f:
        f.write('Filename,CB_Distance,L_Chain_Length\n')
        for filename, distance, chain_length in results:
            f.write(f'{filename},{distance},{chain_length}\n')
