...
class ForceFieldMinimizer(object):
    ...
    def _minimize(self, pdb_str, out_path):
        try:
            ...
            # Freeze receptor chain by setting their atom mass to zero
            # Hydrogens are exception, because fixer add bad position for Hs
            ...
            simulation = openmm_app.Simulation(pdb.topology, system, openmm.LangevinIntegrator(310, 0.1, 0.001), platform)
    ...
    def __call__(self, pdb_path, out_path, return_info=True, cyclic_chains=None, cyclic_opts=None):
        try:
            ...
            pdb_min_bytes = pdb_min.encode('utf-8')
